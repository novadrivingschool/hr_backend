// src/hr/facilities/facilities.service.ts
import { Injectable, NotFoundException } from '@nestjs/common';
import { InjectRepository } from '@nestjs/typeorm';
import { Repository, FindOptionsWhere } from 'typeorm';
import { DateTime } from 'luxon';
import { Facility } from './entities/facility.entity'; // ajusta ruta si cambia
...
export class FacilitiesService {
  ...
  async findOne(id: string) {
    const row = await this.repo.findOne({ where: { id } as FindOptionsWhere<Facility> });
    if (!row) throw new NotFoundException('Facility no encontrado');
